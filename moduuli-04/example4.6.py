'''Toteutetaan algoritmi piin (π) likiarvon laskemiseksi.
 ...
 Neliön nurkkapisteet ovat tällöin (-1,-1), (1,-1), (1,1) ja (-1,1). Jos neliön sisälle arvotaan satunnaisesti suuri määrä pisteitä,
  osuu niistä myös ympyrän sisälle likimain niin suuri osuus kuin ympyrän pinta-ala on neliön pinta-alasta eli πr^2/4,
   ...
   Lopuksi ohjelma tulostaa piin likiarvon käyttäjälle. (Huomaa, että jokaisesta arvotusta pisteestä (x,y) on helppoa testata onko se yksikköympyrän A sisällä:
    riittää testata, toteuttaako piste epäyhtälön x^2+y^2<1.)'''
import random

point_total = int(input("Anna arvottavien pisteiden määrä"))
...
while counter < point_total:
    ...
    if x**2+y**2<1:
        ...
        counter += 1
while counter == point_total:
    pii_val = 4*(point_in_circle/point_total)
    print(pii_val)
    break